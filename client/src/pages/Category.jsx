      ...
      <div className="container mx-auto px-4 py-6">
        <div className="rounded-2xl border bg-white p-5">
          <div className="text-sm text-slate-500 mb-2">
            <button onClick={() => nav(-1)} className="hover:underline">Назад</button>
          ...
          <Link to="/" className="mt-3 inline-flex px-4 py-2 rounded-lg border hover:bg-slate-50">
            На главную
          </Link>
        </div>
      </div>
    ...
    <div className="container mx-auto px-4 py-6 space-y-6">
      ...
      <header className="rounded-2xl border bg-white p-4 md:p-5 flex items-center gap-4">
        <div className="shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-2xl border bg-slate-50 grid place-items-center overflow-hidden">
          <img src={cat.img} alt={cat.title} className="w-12 h-12 md:w-14 md:h-14 object-contain" />
        </div>
        <div className="min-w-0">
          <div className="inline-flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-500">
              Подкатегорий: <span className="font-medium text-slate-700">{cat.subs.length}</span>
            </span>
          </div>
      ...
      <div className="rounded-2xl border bg-white p-3 md:p-4">
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          ...
          <div className="text-xs text-slate-500 md:w-56">
            Найдено: <span className="font-medium text-slate-700">{subs.length}</span>
          </div>
        </div>
      </div>
      ...
      <section>
        {subs.length === 0 ? (
          <div className="rounded-2xl border bg-white p-6 text-center">
            <div className="text-slate-700 mb-2">Ничего не найдено по запросу «{q}».</div>
            <button
              onClick={() => setQ("")}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {subs.map((sub) => (
              <Link
                ...
                className="rounded-2xl border bg-white p-4 hover:shadow-md hover:-translate-y-0.5 transition
              ...
              >
                ...
                <div className="text-sm font-semibold line-clamp-2">{sub}</div>
              </Link>
            ))}
          </div>
        )}
